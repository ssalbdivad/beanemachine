...
**2026-09-02T05:49Z**, horizon 2026-09-02 → 2026-09-16 — scored in the reference
...
| Yahoo | public league player pages | "% Ros", the market's price, and (local server only) who is still available in your league | 845 of the pool priced |
...
Multi-position eligibility as *your* league defines it, lineup order, platoon
splits, weather, and bullpen roles. Park factors are a half-step further along and
the state is worth being exact about: `fetchParkFactors` exists in
`src/data/savant.ts`, the multiplier exists in `project.ts`, its weight is 0 — and
**nothing calls the fetcher**, so no capture carries a park index and no caller ever
passes one. Implemented and unwired, rather than fetched and ignored.

Probable starters used to be on that list and no longer are; see §3.5.
...
The same arithmetic explains the other direction. **140 of 1,431 rateable players
...
**Park factors are implemented and unwired.** The multiplier is written, the weight
is 0, and nothing calls `fetchParkFactors` at all, so no capture even carries the
index. Being precise about that matters: "fetched and unused" would overstate how
close it is.
...
first, which on this eligibility is not merely good enough but optimal — every player
has one kind slot plus one catch-all whose eligible set is a superset of it, so the
exchange argument closes. That guarantee is a property of the eligibility, not of the
greed, and it would stop holding the day real multi-position eligibility arrives.
...
against the twelve runs stored on 2 September, bare `verdict.ts` ranked hot-hand first
and called it significant at z +24.77, where the honest pairing has it losing 37-74.
