 ...
 */
const seatEveryone = (legalByCandidate: string[][], seats: string[]): Map<number, number> => {
...
export const planLineup = (input: PlanInput): LineupPlan => {
	...
	const held = seatEveryone(ranked.map(c => c.legal), open)
	...
	const swaps: LineupSwap[] = starts.map((start, i) => {
		...
		const gainHere = r2(start.points - (out?.points ?? 0))
		return {
			start: start.spot.name,
			startPoints: r2(start.points),
			startSlot,
			sit: out?.spot.name ?? null,
			sitPoints: out ? r2(out.points) : null,
			gain: gainHere,
			reason:
				out ?
					`${start.spot.name} projects ${r2(start.points)} points over the horizon at ` +
					`${startSlot} against ${out.spot.name}'s ${r2(out.points)}, and both are ` +
	...
	if (swaps.length + shifts.length > 0 && gain < options.lineupMinGain)
		return {
			starters,
			swaps: [],
			shifts: [],
	...
	return { starters, swaps, shifts, pointsNow, pointsPlanned, gain, emptySlots, skipped, blocked: null }
}
